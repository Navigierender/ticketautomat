...
bool MoneyCalc::canReturnChange(int value, change reservoir) {
    ...
    // This order matters for the custom denominations
    if (reservoir.seventeen < needed.seventeen) return false;
...
change MoneyCalc::intToChange(int value) {
    change result = {0,0,0,0,0,0,0}; // Initialize all members to 0 (including seventeen)
    int cents = value;

    result.seventeen = cents / 17; cents %= 17;
...
change MoneyCalc::calcChange(int paid, int price, change& reservoir) {
    ...
    // Deduct the returned change from the reservoir
    reservoir.seventeen -= change_to_return.seventeen;
